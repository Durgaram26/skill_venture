...
type ProfileForm = { name: string; email: string; phone: string; about: string; emojiTag: string; profileTitle: string };
...
export function ProfileSettingsPage() {
  ...
  useEffect(() => {
    ...
    void api.me().then((data) => {
      if (cancelled) return;
      const next = { name: data.user.name, email: data.user.email, phone: data.user.phone ?? '', about: data.user.profile?.about ?? '', emojiTag: data.user.profile?.emojiTag ?? '', profileTitle: data.user.profile?.profileTitle ?? '' };
      setForm(next);
      setSavedForm(next);
  ...
  async function onSubmit(event: FormEvent) {
    ...
    setSaved(false);
    try {
      const data = await api.updateProfile({ name: form.name.trim(), email: form.email.trim(), phone: form.phone.trim(), about: form.about.trim(), emojiTag: form.emojiTag.trim(), profileTitle: form.profileTitle.trim() });
      const next = { name: data.user.name, email: data.user.email, phone: data.user.phone ?? '', about: data.user.profile?.about ?? '', emojiTag: data.user.profile?.emojiTag ?? '', profileTitle: data.user.profile?.profileTitle ?? '' };
      updateUser(data.user);
      setForm(next);
      setSavedForm(next);
          ...
          {loading ? <p className="py-12 text-sm text-mute">Loading profile…</p> : tab === 'security' ? (
            ...
            <>
              ...
              <div className="mt-7 grid gap-6 md:grid-cols-2">
                ...
                <label className="block text-sm font-semibold text-slate-900"><span className="mb-2 flex items-center gap-2"><Icon><i className="fas fa-id-badge" aria-hidden /></Icon>Profile badge</span><input className="sv-input" value={form.profileTitle} onChange={(e) => setField('profileTitle', e.target.value.slice(0, 40))} placeholder="e.g. Engineer" maxLength={40} /><small className="mt-2 block font-normal text-slate-500">Choose a short title shown next to your name.</small></label>
