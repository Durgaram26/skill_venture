...
export function ProfilePage() {
  ...
  const completion = Math.round(([user.name, user.email, user.phone, user.profile?.avatar].filter(Boolean).length / 4) * 100);
      ...
      <div className="sv-public-profile bg-slate-50">
        <section className="sv-profile-hero overflow-hidden">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-8 px-5 py-10 md:px-10 md:py-14">
            <div className="relative z-10"><p className="text-xs font-bold uppercase tracking-[.16em] text-violet-700">Learner profile</p><h1 className="mt-3 font-display text-4xl font-extrabold tracking-tight text-slate-950 md:text-5xl">Your learning identity</h1><p className="mt-4 max-w-lg text-base leading-7 text-slate-600">A simple profile institutions can understand at a glance.</p></div>
        ...
        <main className="mx-auto grid max-w-6xl items-start gap-6 px-5 py-7 md:px-10 md:py-9 lg:grid-cols-[minmax(0,1.55fr)_minmax(280px,.75fr)]">
          <article className="overflow-hidden rounded-2xl border border-violet-100 bg-white shadow-[0_8px_30px_rgba(76,29,149,.08)]">
            ...
            <div className="px-5 pb-7 md:px-8">
              <div className="-mt-14 flex flex-col gap-5 sm:-mt-16 sm:flex-row sm:items-end sm:justify-between">
                ...
                <Link to="/profile" className="sv-btn-primary self-start sm:self-auto"><i className="fas fa-pen" aria-hidden />Edit profile</Link>
              </div>
              <div className="mt-8 grid gap-7 border-t border-slate-100 pt-7 md:grid-cols-[1fr_260px]">
                ...
                <section className="rounded-2xl bg-slate-50 p-5"><p className="text-xs font-bold uppercase tracking-[.16em] text-slate-500">Contact details</p><p className="mt-4 flex items-start gap-2 break-all text-sm text-slate-700"><i className="fas fa-envelope mt-1 text-[#1455d9]" aria-hidden />{user.email}</p>{user.phone ? <p className="mt-3 flex items-center gap-2 text-sm text-slate-700"><i className="fas fa-phone text-[#1455d9]" aria-hidden />{user.phone}</p> : <p className="mt-3 text-xs text-slate-500">Add a mobile number from profile settings.</p>}</section>
              </div>
            </div>
          </article>
        ...
        <section className="mx-auto grid max-w-6xl gap-6 px-5 pb-10 md:px-10 lg:grid-cols-2">
